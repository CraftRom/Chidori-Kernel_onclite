...
DEVICE="Redmi 7/Y3"
KERNEL_NAME="Chidori-Kernel-LTS-$TYPE"
ZIP_NAME="Chidori-Kernel-onclite-$(date '+%Y%m%d%H%M')-$TYPE"
TC_DIR="$HOME/toolchains/clang-greenforce"
DEFCONFIG="onclite-perf_defconfig"
...
if [[ ! -d "$TC_DIR" ]]; then
	echo -e "${GREEN} \nGreenforce Clang not found! Cloning to $TC_DIR...\n ${NOCOLOR}"
	if ! git clone --depth=1 https://github.com/greenforce-project/clang-llvm $TC_DIR; then
...
desc
    ...
    make -j$(nproc) O=out ARCH=arm64 CC=clang AS=llvm-as AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- CLANG_TRIPLE=aarch64-linux-gnu- Image.gz-dtb dtbo.img
