...
desc
    ...
    # Export defconfig
    echo -e "${BLUE}(i)     Make DefConfig\n ${NOCOLOR}"
    mkdir -p out
    make -j$(nproc) O=out ARCH=arm64 "$DEFCONFIG"

    # Start kernel compilation
    echo -e "${BLUE}\n(i)     Starting kernel compilation...\n${NOCOLOR}"
	...
	echo -e "${BLUE}\n(i)     Creating ZIP: ${YELLOW}$ZIP_NAME.zip${NOCOLOR}"
    eval $ZIP_CMD
	# Sign the ZIP file with AOSP keys
    echo -e "${BLUE}(i)     Signing zip with aosp keys...\n${NOCOLOR}"
    ...
    SHA=$(shasum "$ZIP_NAME-signed.zip" | cut -d' ' -f1)
    MD5=$(md5sum "$ZIP_NAME-signed.zip" | cut -d' ' -f1)
...
<b>SHA1 checksum:</b> <code>$SHA</code>
...
#onclite #onc #kernel"
    ...
    echo -e "${BLUE}-->    Flashable zip generated  : ${YELLOW}$ZIP_NAME-signed.zip${NOCOLOR}"
    echo -e "${BLUE}-->    MD5 Checksum             : ${YELLOW}$MD5 ${NOCOLOR}"
    echo -e "${BLUE}-->    SHA1 Checksum            : ${YELLOW}$SHA ${NOCOLOR}\n"
else
    # Output the error message and push notification
    echo -e "${RED}\n(i)     Kernel Compilation failed! Fix the errors!\n${NOCOLOR}"
